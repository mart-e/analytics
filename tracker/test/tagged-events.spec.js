...
test.describe('tagged-events extension', () => {
  test('tracks a tagged link click with custom props + url prop', async ({
    page
  }) => {
    ...
    await expectPlausibleInAction(page, {
      ...
      expectedRequests: [
        {
          n: 'Payment Complete',
          p: { amount: '100', method: 'Credit Card', url: linkURL }
        }
      ]
    })
  })

  test('tracks a tagged form submit with custom props when submitting by pressing enter', async ({
    page
  }) => {
    await page.goto('/tagged-event.html')
  ...
  test('tracks submit on a form with a tagged parent when submit button is clicked', async ({
    page
  }) => {
    await page.goto('/tagged-event.html')
  ...
  test('tracks click and auxclick on any tagged HTML element', async ({
    page
  }) => {
    await page.goto('/tagged-event.html')
  ...
  test('does not track elements without plausible-event-name class + link elements navigate', async ({
    page
  }) => {
    await page.goto('/tagged-event.html')
    ...
    expect(
      await plausibleRequestMock,
      'should not have made Plausible request'
    ).toBeNull()
    expect((await navigationRequestMock).url()).toContain(linkURL)
  })

  test('tracks tagged HTML elements when their child element is clicked', async ({
    page
  }) => {
    await page.goto('/tagged-event.html')
    ...
    const expectedRequests = [
      {
        n: 'Custom Event',
        p: { foo: 'bar', url: 'https://awesome.website.com/payment/' }
      }
    ]

    await expectPlausibleInAction(page, {
      action: () =>
        page.click('#h2-with-link-parent', { modifiers: [metaKey()] }),
      expectedRequests
    })
  ...
  test('tracks tagged element that is dynamically added to the DOM', async ({
    page
  }) => {
    await page.goto('/tagged-event.html')
  ...
  test('does not track clicks inside a tagged form, except submit click', async ({
    page
  }) => {
    await page.goto('/tagged-event.html')
