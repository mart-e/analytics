...
test.describe('file-downloads extension', () => {
  test('sends event and does not start download when link opens in new tab', async ({
    ...
    expect(
      await downloadRequestMock,
      'should not make download request'
    ).toBeNull()
  })
  ...
  test('sends File Download event with query-stripped url property', async ({
    page
  }) => {
    await page.goto('/file-download.html')
  ...
  test('starts download only once', async ({ page }) => {
    ...
    const downloadRequestMockList = mockManyRequests({
      page,
      path: downloadURL,
      numberOfRequests: 2
    })
